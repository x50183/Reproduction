        ...
        if len(self._preload) == 0:
            ...
            with lmdb.open(
                ...
                for i in range(self.preload_size):
                    ...
                    _lmdb_value = txn.get(str(self.keys[self.load_ordering.pop()]).encode())
                    new_preload.append(
                        pickle.loads(bytes(_lmdb_value))
                    )
                        ...
                        if args.collect_type in ['TF']:
                            ...
                            for _i, _j in enumerate(train_env.trajectory_id_2_instruction_tokens[infos[i]['trajectory_id']]):
                                ...
                                for k, v in traj_obs.items():
                                    traj_obs[k] = np.ascontiguousarray(v.detach().cpu().numpy())
                                ...
                                train_env.lmdb_features_txn.put(
                                    lmdb_key.encode(),
                                    pickle.dumps(transposed_ep, protocol=pickle.HIGHEST_PROTOCOL),
                                )
